...
func profile() {
	...
	var pages = []page{
		{
			titleImage: logoImg,
			mainText:   "Adam Farmer",
			subText:    "Software Engineer",
			footerText: "LinkedIn /AdamFarmerAU",
			sideImage:  profileImg1,
		},
		{
			titleImage: logoImg,
			mainText:   "Adam Farmer",
		...
		},
		{
			...
			subText:    "Software Engineer",
			footerText: "LinkedIn /AdamFarmerAU",
			sideImage:  profileImg3,
		},
	}

	var currentPage int = -1
	// var ledLit bool = false
	// var sleepTime int = 1000

	if currentPage >= 0 && currentPage < len(pages) {
		drawPage(pages[currentPage])
	}

	// btnA.SetInterrupt(machine.PinToggle, func(pin machine.Pin) {
	// 	currentPage = 0
	// 	drawPage(pages[currentPage])
	// })

	// btnB.SetInterrupt(machine.PinToggle, func(pin machine.Pin) {
	// 	currentPage = 1
	// 	drawPage(pages[currentPage])
	// })

	// btnC.SetInterrupt(machine.PinFalling, func(pin machine.Pin) {
	// 	currentPage = 2
	// 	drawPage(pages[currentPage])
	// })

	for {
		if btnA.Get() && currentPage != 0 {
			currentPage = 0
			drawPage(pages[currentPage])
		}

		if btnB.Get() && currentPage != 1 {
			currentPage = 1
			drawPage(pages[currentPage])
		}

		if btnC.Get() && currentPage != 2 {
			currentPage = 2
			drawPage(pages[currentPage])
		}
...
func drawPage(page page) {
	...
	img := pixel.NewImageFromBytes[pixel.Monochrome](128, 128, []byte(page.sideImage))
	display.DrawBitmap(168, 0, img)

	logo := pixel.NewImageFromBytes[pixel.Monochrome](168, 27, []byte(page.titleImage))
	...
	//tinyfont.WriteLine(&display, &freesans.Bold9pt7b, 8, 22, "Replace this", white)

	var top int16 = 65
	w32, _ := tinyfont.LineWidth(&freesans.Bold24pt7b, page.mainText)
	if w32 < uint32(midW) {
		tinyfont.WriteLine(&display, &freesans.Bold24pt7b, (midW-int16(w32))/2, top, page.mainText, black)
	} else {
		w32, _ := tinyfont.LineWidth(&freesans.Bold18pt7b, page.mainText)
		if w32 < uint32(midW) {
			tinyfont.WriteLine(&display, &freesans.Bold18pt7b, (midW-int16(w32))/2, top, page.mainText, black)
		} else {
			w32, _ := tinyfont.LineWidth(&freesans.Bold12pt7b, page.mainText)
			if w32 < uint32(midW) {
				tinyfont.WriteLine(&display, &freesans.Bold12pt7b, (midW-int16(w32))/2, top, page.mainText, black)
			} else {
				w32, _ := tinyfont.LineWidth(&freesans.Bold9pt7b, page.mainText)
				tinyfont.WriteLine(&display, &freesans.Bold9pt7b, (midW-int16(w32))/2, top, page.mainText, black)
			}
		}
	}

	w32, _ = tinyfont.LineWidth(&freesans.Regular9pt7b, page.subText)
	tinyfont.WriteLine(&display, &freesans.Regular9pt7b, (midW-int16(w32))/2, 102, page.subText, black)

	w32, _ = tinyfont.LineWidth(&freesans.Regular9pt7b, YourSocial)
	if w32 < uint32(midW) {
		tinyfont.WriteLine(&display, &freesans.Regular9pt7b, (midW-int16(w32))/2, 122, page.footerText, black)
	} else {
		w32, _ := tinyfont.LineWidth(&proggy.TinySZ8pt7b, YourSocial)
		tinyfont.WriteLine(&display, &proggy.TinySZ8pt7b, (midW-int16(w32))/2, 120, page.footerText, black)
	}
	display.Display()
	display.WaitUntilIdle()
}
